...
function generateYoloDetections(
  ...
  const isCode = objLower.includes("code") || objLower.includes("display") || objLower.includes("software") || objLower.includes("screenshot") || objLower.includes("interface") || objLower.includes("app") || urlLower.includes("code") || urlLower.includes("screenshot");
  ...
  if (isCode || !isValidCivicIssue && (objLower.includes("screen") || objLower.includes("ui") || objLower.includes("vantage") || objLower.includes("window") || objLower.includes("setting"))) {
    boxes.push({
      label: `Digital Screen / Software UI: ${detectedObject || "Screenshot"}`,
      confidence: 99.2,
      bbox: [8, 8, 92, 92],
      color: "red",
      isFraud: true,
    ...
    boxes.push({
      label: "Non-Civic Digital Application Interface",
      confidence: 97.8,
      bbox: [20, 15, 80, 85],
      color: "red",
      isFraud: true,
  ...
  return {
    ...
    detectedBoxes: boxes,
    cameraMetadata: {
      isOriginalSensor: isValidCivicIssue && !isCode && !isAI && !isGadget,
      deviceType: isCode || isAI || !isValidCivicIssue ? "Digital Screen / Software Screenshot" : "Real Mobile CMOS Camera",
      hasExifGps: true,
    },
  };
}
