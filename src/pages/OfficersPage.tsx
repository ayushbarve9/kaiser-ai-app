...
export const OfficersPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      {/* Banner */}
      <div className="bg-[#0F172A] text-white rounded-xl p-6 border border-slate-800 shadow-xs space-y-3">
        <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded bg-blue-500/20 border border-blue-400/30 text-blue-300 text-xs font-bold uppercase tracking-wider">
          <Award className="w-3.5 h-3.5 text-amber-400" /> Municipal Officer Directory
        </div>
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">
          Mumbai Ward Officers Directory & Performance Matrix
        </h1>
        <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
          Real-time performance metrics for Assistant Municipal Commissioners (AMCs) across all 24 BMC wards, resolution throughput, and official contact helplines.
        </p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            <Building2 className="w-4 h-4 text-blue-600" /> Ward Officer Profile
          </h2>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              <Award className="w-4 h-4 text-amber-500" /> All 24 Ward Officers
            </h2>
            ...
            return (
              <div
                ...
                className={`p-4 rounded-xl border transition-all cursor-pointer space-y-3 ${
                  isSelected
                    ? "bg-slate-50 border-blue-600 shadow-xs ring-1 ring-blue-600"
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <span className="text-xs font-bold text-blue-600 flex items-center justify-end gap-0.5">
                      <Award className="w-3.5 h-3.5 text-amber-500" /> #{w.officer.rank}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 py-2 border-y border-slate-100 text-center text-xs">
                  <div>
                    ...
                    <span className="font-bold text-emerald-600">{w.officer.problemsSolved}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold block">SLA Rate</span>
                    <span className="font-bold text-blue-600">{w.officer.resolutionRate}%</span>
                  </div>
                  <div>
                ...
                <div className="flex items-center justify-between text-xs text-slate-600 pt-0.5">
                  <span className="text-[11px] font-semibold flex items-center gap-1 text-slate-500">
                    <Phone className="w-3 h-3 text-blue-600" /> {w.officer.contact}
                  </span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
