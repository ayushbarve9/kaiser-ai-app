...
import { 
  ...
  CheckCircle2, Sparkles, AlertCircle, Award
} from "lucide-react";
...
export const CitizenLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const loginIdentifier = authMethod === "email" ? email : `${phone.replace(/\D/g, "")}@citizen.civic.com`;
  ...
  const handleQuickCitizenDemo = async (demoEmail: string, demoWard: number) => {
    ...
    setError(null);
    try {
      await login(demoEmail, "password", "Citizen", { ward: demoWard });
      navigate("/");
    } catch (err: any) {
      console.error(err);
      ...
      <div className="w-full max-w-lg space-y-6">
        ...
        <div className="bg-white p-7 sm:p-9 rounded-3xl border border-slate-200/90 shadow-lg space-y-6">
          ...
          <div className="bg-amber-50/70 border border-amber-200/80 p-4 rounded-2xl space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-extrabold text-amber-900 uppercase tracking-wider flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 text-amber-600" /> Quick 1-Click Resident Login
              </span>
              <span className="text-[10px] text-amber-700 font-medium">Instant Access</span>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              <span>{loading ? "Signing in..." : "Enter Citizen Portal"}</span>
