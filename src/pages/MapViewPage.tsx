  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-black text-slate-950 flex items-center gap-2 uppercase tracking-tight">
            <MapPin className="w-5 h-5 text-red-600" />
            Greater Mumbai Geospatial Redressal Map
          </h1>
        ...
        <div className="flex items-center gap-2 flex-wrap text-xs">
          <select
            ...
            className="bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 font-bold text-slate-800 focus:outline-none focus:border-red-600 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[700px]">
        ...
        <div className="lg:col-span-5 bg-white rounded-3xl border border-slate-200 p-4 flex flex-col h-full overflow-hidden shadow-sm">
          <div className="pb-3 mb-3 border-b border-slate-100 flex items-center justify-between text-xs font-bold text-slate-700">
            <span>Mapped Incidents ({complaints.length})</span>
            <span className="text-slate-400 font-normal">Click card to locate on map pin</span>
          </div>
            ...
            {complaints.map((item) => (
              <div
                ...
                className={`transition-all rounded-2xl ${
                  activeComplaint?.id === item.id ? "ring-2 ring-red-600 scale-[0.99]" : ""
                }`}
              >
